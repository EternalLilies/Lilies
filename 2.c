...
int main(){
	...
	if((a==1&&b==0)||(a==2&&b==0)||((a==n-1||a==n-2)&&b==m)||(a==n-1&&b==m+1)||(a==n&&(b==m+1||b==m+2))){
		q=p1+p4;
	}
	if((a==0&&b==2)||(a==0&&b==1)||(a==n&&(b==m-2||b==m-1))||(a==n+1&&b==m-1)||((a==n+1||a==n+2)&&b==m)){
		q=p2+p3;
	}
	...
	if(a==n-2&&b==m+2){
		q=combination(n+m-1,n);
	}
	if(a==n+2&&b==m-2){
		q=combination(n+m-1,n-1);
	}
	printf("%llu\n",q);
	return 0;
}
